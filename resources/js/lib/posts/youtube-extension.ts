 ...
 *   extract an ID → blank shell with no iframe (paste appears to do nothing)
 *
...
export function buildYoutubeEmbedSrc(
    ...
    const base =
        options.nocookie === false ? 'https://www.youtube.com/embed/' : 'https://www.youtube-nocookie.com/embed/';
    ...
    const start =
        typeof options.start === 'number' && options.start > 0 ? options.start : extractYoutubeStartSeconds(raw);
...
export function createCanvasYoutubeExtension() {
    return Youtube.extend({
        ...
        renderHTML({ HTMLAttributes, node }) {
            ...
            if (embedSrc === null) {
                const fallback = typeof rawSrc === 'string' && rawSrc.trim() !== '' ? rawSrc.trim() : null;
