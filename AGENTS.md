...
- **Canvas 7 is a clean break.** No supported upgrade path from prior majors. **Do not** code for backward compatibility, legacy APIs, dual code paths, or “works with v6” fallbacks. Prefer delete-and-replace over shims.
- **One schema migration only:** `database/migrations/2020_09_21_000000_create_canvas_tables.php`. **Edit that file in place** for any schema change. **Never** add a second migration, alter migration, or data-migration class for package schema evolution. Hosts reinstall / drop `canvas_*` per UPGRADE.
...
- Keep the distraction-free writing philosophy intact. **Do not** preserve old public APIs or schema for compatibility during v7 work.
...
- **Page header actions** — top-right create/upload/invite controls use Catalyst **`outline`** (quiet chrome). List **empty-state** CTAs stay solid `color="dark/zinc"` (one focal action when empty). Domain icons on list headers (`IconPlus` create, `IconUpload` media); Dashboard header New post is text-only outline.
- **One create affordance when empty** — true cold empty (`shouldShowEmpty` + no active filters; Dashboard cold audience): hide the quiet header create/upload/invite so the empty-state solid CTA is the only primary action. When the list has items or is filter-empty, show the outline header action. Hide header create during initial skeleton too (avoids flash-then-remove on cold load).
