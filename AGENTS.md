...
- **i18n:** change `resources/lang/en/app.php` first, then **every** locale with real translations and identical keys. Wire keys before inventing duplicates. **No dead keys:** every catalog entry must be referenced from product code (SPA `t('…')`, PHP `trans('canvas::app.…')`, or a documented dynamic builder such as `locale.${code}` / `dashboard.greeting_${period}`). When you remove UI copy or rename a key, delete the old key from **en and every locale** in the same change — do not leave unused strings behind “for later.”
...
- Keep orphaned/unused keys in `resources/lang/**/app.php` after UI removals or renames — prune them from every locale when the call site goes away.
