...
5. Run `php artisan migrate --seed` to fill up the database with minimal data
6. Run `php artisan canvas:install` and follow the on-screen prompts.
7. To build the search index, run `php artisan canvas:index`
8. Change the permissions of the `storage` directory: `sudo chmod -R 777 storage/`
